...
from src.core.database.models import CollectedData, Conversation, Customer, Review
        ...
        "data": [
            {
                ...
                "platform": (
                    conv.platform.value if hasattr(conv.platform, "value") else str(conv.platform)
                ),
                "content": conv.content[:200],  # 截断长内容
                "status": conv.status,
                "priority": (
                    conv.priority.value if hasattr(conv.priority, "value") else str(conv.priority)
                ),
...
async def get_conversation_detail(
    ...
    # 获取收集的数据
    collected_data = (
        db.query(CollectedData).filter(CollectedData.conversation_id == conversation_id).all()
    )
    ...
    return {
        "conversation": {
            ...
            "platform": (
                ...
                if hasattr(conversation.platform, "value")
                else str(conversation.platform)
            ),
            ...
            "received_at": (
                conversation.received_at.isoformat() if conversation.received_at else None
            ),
...
async def get_statistics(
    ...
    return {
        ...
        "by_platform": {str(platform): count for platform, count in platform_stats},
    }
